...
#'  from the cutoff date or biological record that you want to include in your 
#'  climate window search.
#'@param STAT The aggregate statistic used to analyse the climate data Xvar. Can 
...
#'@param STAT2 The aggregate statistic used to analyse the climate data Xvar2. Can 
#'  currently use basic R statistics (e.g. mean, min), as well as slope. 
#'  Additional aggregate statistics can be created using the format function(x) 
#'  (...). See FUN in \code{\link{apply}} for more detail.
...
#'                   STAT = "mean", STAT2 = "sum", FIXED = FALSE,
#'                   CMISSING = FALSE, CINTERVAL = "D")
#'                   
#' # View and plot the output #
#' head(cross)
...
crosswin <- function(Xvar, Xvar2, CDate, BDate, furthest, closest, 
                  STAT, STAT2, FIXED, cutoff.day, cutoff.month,
        ...
        if (STAT != "slope" || STAT2 != "slope" || n > 1){
          ...
          if (STAT2 == "slope"){ 
            ...
            if (n == 1) {
              ...
              temporary2 <- apply(CMatrix2[, windowclose:windowopen], 1, FUN = STAT2)
            }
          }
          # Run the model
  ...
  plotcor(as.data.frame(MODLIST), TYPE="C")
