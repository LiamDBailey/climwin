...
benchmark_climwin <- function(){
  ...
  bench_results_mixed <- purrr::map_df(.x = window_sizes, .f = \(max_range){
    ...
    output <- bench::mark(
      old = suppressWarnings(slidingwin(xvar = list(Temp = OffspringClimate$Temperature),
      ...
      new_parallel = suppressWarnings(run_slidingwin(xvar = "Temperature",
                                                     cdate = "Date",
                                                     bdate = "Date", 
                                                     bio_data = Offspring,
                                                     baseline = glmer(Offspring ~ poly(climate, 2) + (1|Cohort), family = "poisson", data = bio_data),
                                                     ...
                                                     type = "relative",
                                                     fn = mean, cinterval = "day",
                                                     climate_data = OffspringClimate, coef_fn = fixef, parallel = TRUE)),
      check = FALSE) |> 
      select(expression:total_time) |> 
      mutate(expression = as.character(expression),
             across(.cols = c(min, median, total_time), .fns = as.numeric),
